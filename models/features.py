...
def get_feature_set_SC(tweet, sentimentvalues):
    ...
    if sub_score>0:
        additional_freq["sub_score"] = sub_score*10.0
    if obj_score>0:
        additional_freq["obj_score"] = obj_score*10.0
    if nrof_subwords>0:
...
def get_feature_set_PC(tweet, sentimentvalues):
    ...
    for word in sentimentvalues.keys():
        if sentimentvalues[word][0]>0:
            ...
            pol_score = pol_score + sentimentvalues[word][0]
        if sentimentvalues[word][1]>0:
            nrof_neg_words = nrof_neg_words + 1
            pol_score = pol_score + sentimentvalues[word][0]

    if pol_score>0:
        features['pol_score'] = pol_score*10.0
    if nrof_pos_words>0:
